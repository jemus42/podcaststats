  ...
  mutate(title_long = str_wrap(paste0(number, ": ", title), 20)) %>%
{
  ...
  geom_point(alpha = .8) +
  geom_label_repel(data = filter(., duration > 150), aes(label = title_long), alpha = .7) +
  scale_x_datetime(minor_breaks = date_breaks("3 months")) +
  labs(title = "The Incomparable: Main Show",
