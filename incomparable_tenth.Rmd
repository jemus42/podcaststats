...
Since its first episode on {format(min(as.Date(incomparable_wide$date, na.rm = TRUE)), "%B %d, %Y")}, The Incomparable has accumulated {round(sum(incomparable_wide$duration, na.rm = TRUE)/60)} hours of content (roughly {round(sum(incomparable_wide$duration, na.rm = TRUE)/60/24)} days) across {nrow(incomparable_wide)} episodes.
```
