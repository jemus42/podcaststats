...
In the meantime, I had [taken John's advice](https://twitter.com/siracusa/status/801444008404914176) and started [downloading every ATP episode ever as mp3](https://twitter.com/Jemus42/status/801451942560022528) to manually [mediainfo](https://en.wikipedia.org/wiki/MediaInfo) the duration out of them in a script, only to realize that the first few episode [didn't have title information](https://twitter.com/Jemus42/status/801504118816833536) in them. This left the only option of crawling the ATP website for full episode metadata (which is annoying because of non-trivial URL paremeters for pagination), which I was very much about to do, until I got Marco's reply.  
So… thanks Marco for saving me from the horrible fate that would have been a while loop in R.

## Episode Duration

```{r atp_duration}
ggplot(atp, aes(x = as.POSIXct(date), y = duration)) +
  geom_point() +
