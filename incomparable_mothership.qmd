...
incomparable_wide |>
  ...
  mutate(
    date = as.Date(date),
    guest = stringr::str_replace_all(guest, ";", ", ")
  ) |>
  knitr::kable(col.names = c("#", "Date", "Title", "Host", "Guests")) |>
...
There were {nrow(filter(incomparable_wide, year == current_year))} episodes published in {current_year}. Here's a breakdown by host and guest counts.
```

```{r current_year_panelists_tab, fig.height=9}
panelist_breakdown <- incomparable |>
...
```{r current_year_panelists_plot, fig.height=9}
