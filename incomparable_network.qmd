...
incomparable_episodes |>
  ...
  scale_y_time(
    breaks = hms::hms(minutes = seq(0, 300, 30))
  ) +
...
All in all, *The Incomparable* published `r round(sum(incomparable_episodes$duration) / lubridate::dminutes(1))` minutes of content, or `r round(sum(incomparable_episodes$duration) / lubridate::dweeks(1))` weeks, which are numbers that exists as a concept in your head but not in reality.
