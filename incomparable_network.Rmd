...
## 2020 Hosts and Guests

```{r 2020_panelists_network, fig.height=14}
incomparable_master %>%
...
```{r 2020_panelists_network_fulltable}
incomparable_master %>%
  filter(as.numeric(as.character(year)) == 2020) %>%
  select(
    podcast, number, date,
...
incomparable_master %>%
  ...
  labs(
    title = "The Incomparable Network — 2020",
...
```{r incomparable_network_2020_shows}
incomparable_master_wide %>%
  filter(as.numeric(as.character(year)) == 2020) %>%
  count(podcast) %>%
  ggplot(aes(x = reorder(podcast, n), y = n)) +
...
In 2020, *The Incomparable* published `r as.character(dminutes(sum(incomparable_master_wide$duration[incomparable_master_wide$year == "2020"])))` of content. That's a lot. In case you were wondering what time is.

```{r incomparable_network_2020_episodes, fig.height=6, fig.width=9}
incomparable_master_wide %>%
  filter(as.numeric(as.character(year)) == 2020) %>%
