...
incomparable_master_wide %>%
  ...
  geom_col() +
  coord_flip() +
  labs(title = "The Incomparable Network",
...
## 2019 Hosts and Guests

```{r 2019_panelists_network, fig.height=14}
incomparable_master %>% 
  filter(year == 2019) %>%
...
Here's a searchable and sortable list of all the episodes released in 2019 (UTC, no freedom timestamps, sorry). There are multiple rows per episode, because each person gets their own row, which should make it a little easier to find out which person aprticipated in which episode. Because manually searching through <https://www.theincomparable.com/person/> would be almost like work.

```{r 2019_panelists_network_fulltable}
incomparable_master %>% 
  filter(year == 2019) %>%
...
As a bonus, here's a proportional chart of all people having appeared on at least 10 occastions throughout 2019. Unfortunately, it's pretty hard to make out individual podcasts because there just aren't as many distinctive colors as there are shows on the Incomaparable network :(


```{r}
incomparable_master %>%
  filter(year == 2019) %>%
  count(podcast, person) %>%
  group_by(person) %>%
  mutate(total = sum(n)) %>%
  filter(total >= 10) %>%
  mutate(p = n / total) %>%
  ggplot(aes(x = reorder(person, total), y = p, fill = podcast)) +
  geom_col() +
  coord_flip() +
  scale_y_percent() +
  scale_fill_viridis(discrete = TRUE, option = "A") +
  labs(title = "The Incomparable Network — 2019",
       subtitle = "Relative Show Appearances by Person",
       x = "", y = "", fill = "Show",
       caption = caption)
```
...
```{r incomparable_network_2019_shows}
incomparable_master_wide %>%
  filter(year == 2019) %>%
  count(podcast) %>%
  ggplot(aes(x = reorder(podcast, n), y = n)) +
...
In 2019, *The Incomparable* published `r as.character(dminutes(sum(incomparable_master_wide$duration[incomparable_master_wide$year == "2019"])))` of content. That's a lot. In case you were wondering what time is.

```{r incomparable_network_2019_episodes, fig.height=6, fig.width=9}
incomparable_master_wide %>%
  filter(year == 2019) %>%
...
All in all, *The Incomparable* published `r as.character(dminutes(sum(incomparable_master_wide$duration)))` of content, which is a number that exists as a concept in your head but not in reality.
...
incomparable_master_wide %>%
  ...
  count(year) %>%
  {
    ggplot(data = ., aes(x = year, y = n)) +
      ...
      annotate("text", label = paste("bar(x) ==", round(mean(.$n), 1)), parse = TRUE,
               x = "2011", y = 300) +
      labs(title = "The Incomparable Network — 2018",
