---
title: "The Incomparable: Network Wide"
date: "`r format(Sys.time(), '%F %R', tz = 'UTC', usetz = T)`"
author: "Lukas — @jemus42"
output: 
  html_document:
    fig_height: 8
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
library(knitr)
#### Data prep ####
source("setup.R")

#### Global options ####
options(max.print = "75")
opts_chunk$set(echo    = FALSE,
	             cache   = FALSE,
               prompt  = FALSE,
               tidy    = TRUE,
               comment = NA,
               message = FALSE,
               warning = FALSE,
               fig.retina = 2,
               fig.path = "assets/plots/incomparable_network/")
opts_knit$set(width = 75)

incomparable_master_wide <- readRDS("data/incomparable_master_wide.rds")
incomparable_master      <- readRDS("data/incomparable_master.rds")
```

## All Time Stats

All shows on the network, first and last episode timeline.

```{r incomparable_network_timeline, fig.height=8}
incomparable_master_wide %>%
  group_by(podcast) %>%
  mutate(d_min = min(date),
         d_max = max(date)) %>%
  ggplot(aes(x = reorder(podcast, d_min))) +
  geom_errorbar(aes(ymin = d_min, ymax = d_max),
                width = .5, size = 2, alpha = .75, color = "black") +
  coord_flip() +
  scale_x_discrete(position = "top") +
  labs(x = "", y = "First to Last Episode Date",
       title = "The Incomparable Network", 
       subtitle = "Show Timelines",
       caption = caption)
```


All episodes since 2011.

```{r incomparable_network_alltime}
incomparable_master_wide %>%
  count(podcast) %>%
  ggplot(aes(x = reorder(podcast, n), y = n)) +
  geom_col() +
  coord_flip() +
  labs(title = "The Incomparable Network",
       subtitle = "All Episodes Since 2011 by Show",
       x = "", y = "# of Episodes",
       caption = caption)

incomparable_master_wide %>%
  filter(year >= 2011) %>%
  ggplot(data = ., aes(x = date, y = duration, fill = podcast)) +
  geom_point(shape = 21, color = "black", alpha = .5) +
  scale_y_continuous(breaks = seq(0, 300, 30), 
                     minor_breaks = seq(0, 300, 15)) +
  scale_fill_viridis(option = "A", discrete = TRUE, guide = FALSE) +
  labs(title = "The Incomparable Network",
       subtitle = "All Episodes Since 2011 by Duration, Colored by Show",
       x = "", y = "Duration (mins)",
       caption = caption)

incomparable_master_wide %>%
  filter(year >= 2011) %>%
  count(year, month) %>%
  ggplot(data = ., aes(x = month, y = n)) +
  geom_point(color = "black", alpha = .75) +
  facet_wrap(~year, nrow = 1, scales = "free_x") +
  labs(title = "The Incomparable Network",
       subtitle = "Episodes per Month Since 2011",
       x = "", y = "# of Episodes",
       caption = caption) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = rel(0.5)))
```

Episode duration by the 15 people most frequently appearing on the network:

```{r top_15_people_duration}
incomparable_master %>%
  filter(year >= 2011) %>%
  count(person) %>%
  top_n(15) %>%
  pull(person) -> top_15

incomparable_master %>%
  filter(person %in% top_15) %>%
  group_by(person) %>%
  mutate(mean_duration = mean(duration)) %>%
  ggplot(data = ., aes(x = reorder(person, mean_duration), y = duration)) +
  geom_point(alpha = .5, position = position_jitter(height = 0, width = .25)) +
  geom_boxplot(alpha = .5, outlier.alpha = 0) +
  coord_flip() +
  labs(title = "The Incomparable Network",
       subtitle = "15 Most Frequently Appearing People's Episode Duration",
       x = "", y = "Duration (mins)",
       caption = caption)
```


## 2018 Hosts and Guests

```{r 2018_panelists_network, fig.height=14}
incomparable_master %>% 
  filter(year == 2018) %>%
  group_by(person, role) %>%
  tally() %>%
  spread(role, n) %>% 
  replace_na(list(guest = 0, host = 0)) %>%
  mutate(total = guest + host) %>%
  arrange(desc(total)) %>%
  datatable(style = "bootstrap", colnames = c("Person", "Guest", "Host", "Total Appearances"))
```

I opted to leave out the plot for that data because *oh boy so many people*, so I hope everyone will be happy with the table.  
It was artisanally crafted as a gift to *The Incomparable* panelists and fans <3

### A Searchable Table of All The nice People<small>*</small>

<small>* All the nice people _and_ Philip Mozolak</small>

Here's a searchable and sortable list of all the episodes released in 2018 (UTC, no freedom timestamps, sorry). There are multiple rows per episode, because each person gets their own row, which should make it a little easier to find out which person aprticipated in which episode. Because manually searching through <https://www.theincomparable.com/person/> would be almost like work.

```{r 2018_panelists_network_fulltable}
incomparable_master %>% 
  filter(year == 2018) %>%
  select(podcast, number, date, 
         duration, title, person, role) %>%
  mutate(duration = round(duration, 2),
         date = lubridate::as_date(date),
         podcast = factor(podcast)) %>%
  arrange(desc(date)) %>%
  datatable(style = "bootstrap", filter = "top",
            colnames = c("Show", "#", "Date (UTC)",
                         "Duration (mins)", "Title", "Person", 
                         "Role")) %>%
  formatDate(3, method = "toDateString")
```

As a bonus, here's a proportional chart of all people having appeared on at least 10 occastions throughout 2018. Unfortunately, it's pretty hard to make out individual podcasts because there just aren't as many distinctive colors as there are shows on the Incomaparable network :(


```{r}
incomparable_master %>%
  filter(year == 2018) %>%
  count(podcast, person) %>%
  group_by(person) %>%
  mutate(total = sum(n)) %>%
  filter(total >= 10) %>%
  mutate(p = n / total) %>%
  ggplot(aes(x = reorder(person, total), y = p, fill = podcast)) +
  geom_col() +
  coord_flip() +
  scale_y_percent() +
  scale_fill_viridis(discrete = TRUE, option = "A") +
  labs(title = "The Incomparable Network — 2018",
       subtitle = "Relative Show Appearances by Person",
       x = "", y = "", fill = "Show",
       caption = caption)
```


## 2018 Shows

TL;DR TeeVee is basially cheating.

```{r incomparable_network_2018_shows}
incomparable_master_wide %>%
  filter(year == 2018) %>%
  count(podcast) %>%
  ggplot(aes(x = reorder(podcast, n), y = n)) +
  geom_col() +
  coord_flip() +
  labs(title = "The Incomparable Network — 2018",
       subtitle = "Shows by episode count",
       x = "",
       y = "# of Episodes",
       caption = caption)
```

## 2018 Episodes

In 2018, *The Incomparable* published `r as.character(dminutes(sum(incomparable_master_wide$duration[incomparable_master_wide$year == "2018"])))` of content. That's a lot. In case you were wondering what time is.

```{r incomparable_network_2018_episodes, fig.height=6, fig.width=9}
incomparable_master_wide %>%
  filter(year == 2018) %>%
  mutate(date = round_date(date, unit = "week")) %>%
  count(date) %>%
  {
    ggplot(data = ., aes(x = date, y = n)) +
    geom_col() +
    geom_hline(yintercept = mean(.$n), color = "red", size = 1, lty = "dotdash") +
      annotate("text", label = paste("bar(x) ==", round(mean(.$n), 1)), parse = TRUE,
               x = as.POSIXct("2018-01-5"), y = 13.5) +
    scale_x_datetime(date_breaks = "1 month",
                     date_labels = "%B") +
      labs(title = "The Incomparable Network — 2018",
           subtitle = "Shows per week",
           x = "",
           y = "# of Episodes per Week",
           caption = caption) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  }
```

## Yearly Episode Output

All in all, *The Incomparable* published `r as.character(dminutes(sum(incomparable_master_wide$duration)))` of content, which is a number that exists as a concept in your head but not in reality.

```{r incomparable_network_episodes_peryear}
incomparable_master_wide %>%
  filter(year >= 2011) %>%
  count(year) %>%
  {
    ggplot(data = ., aes(x = year, y = n)) +
      geom_col() +
      geom_hline(yintercept = mean(.$n), color = "red", size = 1, lty = "dotdash") +
      annotate("text", label = paste("bar(x) ==", round(mean(.$n), 1)), parse = TRUE,
               x = "2011", y = 300) +
      labs(title = "The Incomparable Network — 2018",
           subtitle = "Shows per year",
           x = "",
           y = "# of Episodes per Year",
           caption = caption)
  }
```

